...
int main(int argc, char** argv)
{
  ...
  UpperConfidenceBound<GaussianProcess<RBFKernel>> strategy;
  Rosenbrock func;
  std::vector<Eigen::Vector2d> bounds{{-2.0, 2.0}, {-2.0, 2.0}};
  BayesianOptimization<GaussianProcess<RBFKernel>, UpperConfidenceBound<GaussianProcess<RBFKernel>>>
      opt(gp, strategy, bounds, 100, 200, 100);
  Eigen::Vector2d ans = opt.optimize<Rosenbrock>(func, Eigen::Vector2d(1.7, -0.5), true);
